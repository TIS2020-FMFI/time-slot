...
/// ASI TU JE CHYBAAAAAA
$sql = "select  start_date_time,end_date_time,(select CONCAT(email,' ',meno_splocnosti) as full_name from employee where id = id_external_dispatcher)from time_slot where start_date_time >= TIMESTAMP( '{$date}' ) and id_external_dispatcher != 0"; // delete from `time_slot` where start_date_time  >=  TIMESTAMP($date)
if ($result = $mysqli->query($sql)) {  // vykonaj dopyt
    $vysl =  $result->fetch_all();
    echo json_encode($vysl).'<br>'; // koli tomu aby sa dalo inforomovat o odstraneni time slotov danich userov
} else{
